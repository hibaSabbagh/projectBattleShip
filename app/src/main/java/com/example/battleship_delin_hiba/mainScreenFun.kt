...
fun MainScreen(
    ...
    if (model.localPlayerId.value == null) {
        ...
        val playerInvalid =
            playerName.isEmpty() || players.any { it.value.name == playerName } || !(playerName.matches(
                Regex("^[a-zA-Z]*")
            ))

        Scaffold(
            floatingActionButton = {
                ExtendedFloatingActionButton(                  //för join lobby knappen
                    onClick = {
                        isJoining =
                            true                                                   //när knappen trycks på så ändras flaggan till true
                        if (!playerInvalid) {
                            handleJoinGame(
                                navController,
                                model,
                                playerName,
                                sharedPreferences
                            )              //anropar handleJoinGame om namnet är giltigt
                        }
                    },
            ...
            Column(
                ...
                Box(
                    modifier = Modifier
                        .fillMaxWidth(0.8f)
                        .background(Color(0xFFD986AC), RoundedCornerShape(4.dp))
                        .padding(16.dp)
                ) {
                    Column {
                        TextField(
